                                          ...
                                          int& theHeight)
{
  ...
  if (bottomleft.X() == kFloatMissing || bottomleft.Y() == kFloatMissing ||
      topright.X() == kFloatMissing || topright.Y() == kFloatMissing)
  {
    ...
    NFmiStereographicArea area(theCenter,
                               theCenter,
                               theCentralLongitude,
                               NFmiPoint(0, 0),
                               NFmiPoint(1, 1),
    ...
    NFmiPoint bl(c.X() - theScale * 1000 * static_cast<float>(theWidth),
                 c.Y() - theScale * 1000 * static_cast<float>(theHeight));
    NFmiPoint tr(c.X() + theScale * 1000 * static_cast<float>(theWidth),
                 c.Y() + theScale * 1000 * static_cast<float>(theHeight));
