...
void draw_roundarrow(NFmiImage &img, const NFmiPoint &xy, float speed, float angle)
{
                         ...
                         NFmiDataMatrix<float> &direction)
{
  ...
  else
  {
    ...
    std::shared_ptr<NFmiDataMatrix<NFmiPoint>> latlon = globals.queryinfo->Locations();
